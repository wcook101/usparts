...
function SellerLogoStrip({ sellers }: { sellers: FeaturedSeller[] }) {
  return (
      ...
      <div className="flex min-w-max gap-3 sm:min-w-0 sm:flex-wrap sm:justify-center">
        {sellers.map((seller) => (
          <Link
            ...
            className="group flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm transition hover:border-blue-200 hover:shadow-md"
          >
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-sm font-bold text-white transition group-hover:bg-blue-600">
            ...
            <span className="min-w-0">
              ...
              <span className="block text-xs text-slate-500">
                {formatQuantity(seller.listingCount)} listings
              </span>
            </span>
          </Link>
        ))}
...
export function FeaturedSellersSection({ sellers }: FeaturedSellersSectionProps) {
  return (
    <section className="relative border-b border-slate-200 bg-slate-50/80">
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-16">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-600 sm:text-base">
              Registered US electronic component suppliers with verified accounts
              and active inventory listed for free BOM search.
            </p>
          </div>
          <Link
            href="/company"
            className="inline-flex w-fit rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-slate-800 ring-1 ring-slate-200 hover:bg-slate-50"
          >
        ...
        {sellers.length > 0 ? (
          <>
            <SellerLogoStrip sellers={sellers} />
              ...
              {sellers.map((seller) => (
                ...
                >
                  ...
                  <p className="mt-4 text-sm text-slate-600">
                    ...
                    active {seller.listingCount === 1 ? "listing" : "listings"}{" "}
                    searchable by MPN
                  </p>
